...
主题文件在技能的`themes`文件夹中, 一共 18 个`json`主题文件,  
用户可以随意修改, 或者让 AI 生成新的主题文件, 数据结构与已有主题文件保持一致即可.

系统提供 18 种文章渲染主题，每种主题都有独特的视觉风格：
...
- **dark（暗黑主题）**：深邃暗黑主题，绿色标签标题 + 暗色代码块。**此主题在 skill 场景下会自动预处理**（见下方说明）

### dark 主题专属预处理

dark 主题的二级标题（绿色实心填充标签）和三级标题（绿色方块/下划线）通过 HTML 内联样式实现，
在**通过本 skill 发布**、并且 `theme: "dark"` 时，会自动完成预处理，无需用户手动修改 `markdownFilePath`。

**在 skill 场景下的推荐流程：**

- 用户只需提供原始 Markdown 路径，并在配置中将 `theme` 设为 `dark`；
- skill 会自动选择使用项目内的 `publish.mjs` 作为入口：
  - 内部先调用 `preprocess-dark.mjs`，对原始 Markdown 执行预处理；
  - 生成 `_processed_*.md` 临时文件并写入临时 `config`；
  - 再调用 `index.js` 发布到公众号草稿箱；
  - 预处理过程中会跳过代码块内的 `##` / `###`，避免误转换。

**仅在手动命令行使用脚本时，才需要自行调用预处理脚本。示例：**

```bash
# 预处理：将 ##/### 转换为 HTML 标题样式
node "<技能目录>/preprocess-dark.mjs" "<原始Markdown文件>" "<处理后输出文件>"

# Windows 示例
node "C:/Users/daman/.agents/skills/wechat-publisher-yashu/preprocess-dark.mjs" "D:/文章/my-article.md" "D:/文章/_processed_my-article.md"
```

预处理脚本会自动：
- 将 `## 标题` 转换为居中的绿色（#10B981）实心填充标签，自适应文字长度
- 将 `### 标题` 转换为绿色（#059669）标题，并在前方增加 `■` 前缀
- 跳过代码块内的 `##`/`###`，不会误转换

预处理后，将 `config.json` 的 `markdownFilePath` 指向**处理后的文件**，再执行发布命令。
