...
- **dark（暗黑主题）**：深邃暗黑主题，绿色标签标题 + 暗色代码块。**此主题需要预处理**（见下方说明）

### dark 主题专属预处理

dark 主题的二级标题（绿色实心填充标签）和三级标题（蓝色方块+下划线）通过 HTML 内联样式实现，需要在发布前对 Markdown 进行预处理。

**发布 dark 主题文章时，必须在执行发布命令之前先运行预处理脚本：**
...
预处理脚本会自动：
- 将 `## 标题` 转换为居中的绿色（#23e89a）实心填充标签，自适应文字长度
- 将 `### 标题` 转换为蓝色（#02A8F9）方块前缀 + 底部下划线样式
- 跳过代码块内的 `##`/`###`，不会误转换
